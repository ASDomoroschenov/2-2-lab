...
bigint *bigint_concrete::multiply(
        const bigint *target_mult)
{
    return execute_operation(target_mult, OPERATION::MULTIPLY);
}
...
bigint *bigint_concrete::divide(
        const bigint *target_div)
{
    return execute_operation(target_div, OPERATION::DIVIDE);
}

bigint *bigint_concrete::division(
        const bigint *target_div) const
{
    bigint *new_obj = new bigint_concrete(this);

    return *new_obj /= target_div;
}
