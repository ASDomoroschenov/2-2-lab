...
void *allocator_descriptor::allocate(size_t const &size) const
{
    size_t size_to_allocate = size + sizeof(size_t) + 2 * sizeof(void*);
...
void allocator_descriptor::deallocate(void *deallocated_block) const
{
    ...
    auto **next_deallocated_block = reinterpret_cast<void**>(deallocated_block) - 1;
    ...
    if (log_memory != nullptr)
    {
        log_memory->log("deallocated block (" + to_string(deallocated_block) + ") : " + get_bytes_descriptor(address_deallocated_block), logger::severity::INFORMATION);
    }
