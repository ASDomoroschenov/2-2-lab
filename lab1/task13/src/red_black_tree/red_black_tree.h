...
protected:
    ...
    private:
        ...
        red_black_tree<tkey, tvalue, tkey_comparer> *_tree;
        red_black_node *_information_deleted_node;
...
public:
    ...
    void for_red_black_tree() override;
...
template<
        ...
        typename tvalue,
        typename tkey_comparer>
void red_black_tree<tkey, tvalue, tkey_comparer>::bypass(
        typename binary_search_tree<tkey, tvalue, tkey_comparer>::bin_node *node)
{
    if (node != nullptr)
    {
        bypass(node->left_subtree);

        std::string arr[2] = {"RED","BLACK"};
        std::cout << node->key << " " << arr[static_cast<int>(reinterpret_cast<red_black_node*>(node)->color)] << std::endl;

        bypass(node->right_subtree);
    }
}
...
template<
        ...
        typename tvalue,
        typename tkey_comparer>
void red_black_tree<tkey, tvalue, tkey_comparer>::for_red_black_tree()
{
    bypass(this->_root);
}
...
void red_black_tree<tkey, tvalue, tkey_comparer>::red_black_removing_template_method::get_info_deleted_node(
        ...
        std::stack<typename binary_search_tree<tkey, tvalue, tkey_comparer>::bin_node**> &path_to_subtree_root_exclusive)
{
    _information_deleted_node = new red_black_node;
            ...
            if (subtree_root_address->right_subtree != nullptr)
            {
                ...
                if (color_sibling == red_black_tree<tkey, tvalue, tkey_comparer>::color_node::BLACK)
                {
                        ...
                        color_children_right == red_black_tree<tkey, tvalue, tkey_comparer>::color_node::BLACK)
                    {
                        ...
                        {
                            _information_deleted_node = reinterpret_cast<red_black_node*>(subtree_root_address);
