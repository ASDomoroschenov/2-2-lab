...
allocator_list::allocator_list(
        size_t const &size,
        memory *outer_allocator_list,
        logger *logger_allocator_list,
        memory::METHOD_SUITABLE method_allocation)
{
    _method = method_allocation;

    if (outer_allocator_list)
    {
        _allocated_memory = outer_allocator_list->allocate(size);
    }
    else
    {
    ...
    *outer_allocator_list_space = outer_allocator_list;

    auto **logger_space = reinterpret_cast<logger**>(outer_allocator_list_space + 1);
    *logger_space = logger_allocator_list;
...
//begin region get suitable block
...
//end region get suitable block


//begin region allocate

void *allocator_list::allocate(
...
//begin region deallocate

std::string get_bytes_list(
        void *object)
{
    std::string result;
...
void allocator_list::deallocate(
        void *deallocated_block) const
{
    ...
    void *current_free_block = get_first_free_block();
    void *address_deallocated_block = reinterpret_cast<size_t*>(deallocated_block) - 1;
    void *prev_free_block = nullptr;
    logger *log_memory = get_logger();

    if (log_memory != nullptr)
    {
        log_memory->log("deallocated block (" + to_string(deallocated_block) + ") : " + get_bytes_list(address_deallocated_block), logger::severity::INFORMATION);
    }
...
void allocator_list::compact_memory() const
{
    void *current_free_block = get_first_free_block();
