        ...
        void *object)
{
    ...
    size_t size_object = *reinterpret_cast<size_t*>(object - 2 * sizeof(void*) - sizeof(size_t));
...
void *allocator::get_next_occupied_block(
...
size_t allocator::get_block_size(
...
void *allocator::get_first_suitable_block(
        ...
        while (current_occupied_block != nullptr)
        {
            ...
            if (next_occupied_block != nullptr)
            {
                ...
                if (size + sizeof(size_t) + 2 * sizeof(void*) <= distance)
                {
                    ...
                    *reinterpret_cast<void**>(current_occupied_block + sizeof(size_t)) = allocated_block;
                    *reinterpret_cast<void**>(next_occupied_block + sizeof(size_t) + sizeof(void*)) = allocated_block;
...
void *allocator::get_best_suitable_block(
        size_t const &size) const
{
    void *current_occupied_block = get_first_occupied_block();
    void *allocated_block = nullptr;
    ...
    else
    {
        ...
        *reinterpret_cast<void**>(prev_block + sizeof(size_t) + sizeof(void*)) = allocated_block;
        ...
        {
            *reinterpret_cast<void**>(next_block + sizeof(size_t) + sizeof(void*)) = allocated_block;
        }
...
void *allocator::get_worse_suitable_block(
        size_t const &size) const
{
    void *current_occupied_block = get_first_occupied_block();
    void *allocated_block = nullptr;
    ...
    memory::METHOD_SUITABLE method_allocation)
{
    ...
    *first_occupied_block = nullptr;
    ...
    memory **outer_allocator_space = reinterpret_cast<memory**>(first_occupied_block + 1);
    *outer_allocator_space = outer_allocator;

    logger **logger_space = reinterpret_cast<logger**>(outer_allocator_space + 1);
    *logger_space = logger_allocator;

    log_with_guard("allocator is created", logger::severity::INFORMATION);
    log_with_guard("size of memory allocated for allocator: " + to_string(size), logger::severity::INFORMATION);
}
        ...
        size_t const &size) const
{
    ...
    if (alc == nullptr)
    {
        ...
        log_with_guard("block (" + to_string(allocated_block - sizeof(size_t) - 2 * sizeof(void*)) + ") size allocated from allocator: " + to_string(size), logger::severity::INFORMATION);
    }
    else
    {
        ...
        void *deallocated_block) const
{
    ...
    if (alc == nullptr)
    {
        ...
        {
            *reinterpret_cast<void**>(_allocated_memory + sizeof(size_t)) = nullptr;
        }
    }
    else
    {
    ...
    log_with_guard("deallocated block (" + to_string(deallocated_block - sizeof(size_t) - 2 * sizeof(void*)) + ") : " + get_bytes(deallocated_block), logger::severity::INFORMATION);
}
...
allocator::~allocator()
{
    ...
    log_with_guard("allocator is destroyed", logger::severity::INFORMATION);
